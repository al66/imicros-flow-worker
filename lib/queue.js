...
module.exports = {
    ...
    actions: {
        ...
        add: {
            ...
            async handler(ctx) {
                ...
                // get next index
                let index = await this.nextIndex({ owner, serviceId: ctx.params.serviceId });
                // prepare token
        ...
        fetch: {
            ...
            async handler(ctx) {
                ...
                let index = await this.fetchNext({ owner, serviceId: ctx.params.serviceId, workerId: ctx.params.workerId, timeToRecover });
        ...
        ack: {
            ...
            async handler(ctx) {
                let owner = _.get(ctx,"meta.ownerId",null);
                let index = await this.getCurrent({ owner, serviceId: ctx.params.serviceId, workerId: ctx.params.workerId });
                ...
                try {
                    let result = await this.ack({ owner, serviceId: ctx.params.serviceId, workerId: ctx.params.workerId });
                    // given token ?
        ...
        rewind: {
            ...
            async handler(ctx) {
                ...
                try {
                    let result = await this.rewind({ owner, serviceId: ctx.params.serviceId, last: `${ctx.params.last}` });
                    return result;
                } catch (err) /* istanbul ignore next */ {
                    this.logger.error("Redis rewind error", { error: err.message, params: ctx.params });
                    return false;
                }
            }
        },
        ...
        info: {
            ...
            async handler(ctx) {
                ...
                try {
                    let result = await this.info({ owner, serviceId: ctx.params.serviceId });
                    return result;
                } catch (err) /* istanbul ignore next */ {
                    this.logger.error("Redis request info error", { error: err.message, params: ctx.params });
                    return null;
                }
            }
        }
    ...
    async created() {
        ...
        this.contextTable = _.get(this.settings, "cassandra.queueTable", "queue" );
        ...
        this.services = {
            ...
            activity: _.get(this.settings, "services.activity", "activity" )
        };        
