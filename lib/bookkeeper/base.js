...
const Base  = {
    ...
    methods: {
        ...
        async getCurrent ({ owner, serviceId, workerId }) {
            let key = `${ owner }-${ serviceId }`;
            let current = await this.client.hget(key, workerId);
            if (current) return current.split(":")[0];
            return 0;
        },
        ...
        async rewind ({ owner, serviceId, last }) {
            let key = `${ owner }-${ serviceId }`;
            let index = await this.client.hget(key, "FETCHED");
            if (index && parseInt(index) >= parseInt(last)) return this.client.hset(key, "FETCHED", last);  
            return false;
        },
